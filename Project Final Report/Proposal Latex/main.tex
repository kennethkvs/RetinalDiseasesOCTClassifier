...
The learning rate scheduler CosineAnnealingLR was chosen because it decreases the learning rate according to a cosine curve, which allows for an overall smooth learning rate curve that keeps training stable, allowing for better generalization in the model. As well as the ability to rapidly decrease the learning rate in the beginning when it is usually still far from the optimal value, and slows down near the end when it nears the optimal value,  allowing for faster convergence. In addition to the learning rate scheduler, a prebuilt warmup scheduler function, GradualWarmupScheduler, created by a Kaggle user, was implemented. The warmup scheduler uses the first 5 epochs of the model as a kind of 'warm up', increasing the training rate slowly to the desired value, after which it hands it off to the normal Cosine scheduler to continue the learning rate scheduling. This warm-up helps stabilize the training of the model, which in turn allows for overall better generalization within the model, leading to better performance, and allows for a high beginning learning rate value to be set for the learning rate scheduler, which gives way for faster convergence. Lastly, to help further prevent overfitting, save computation time, and achieve faster convergence, early-stopping was implemented with the monitoring of the validation loss with a threshold value of 0.0001 and patience of 10.
