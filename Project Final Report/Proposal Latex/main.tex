...
The learning rate scheduler of CosineAnnealingLR was chosen because it decreases the learning rate according to a cosine curve, which allows for an overall smooth learning rate curve that keeps training stable, allowing for better generalization in the model. This works in tandem with the ability to rapidly decrease the learning rate in the beginning when it is usually still far from the optimal value, and slows down near the end when it nears the optimal value, allowing for faster convergence.

In addition to the learning rate scheduler, a prebuilt warm-up scheduler function, GradualWarmupScheduler, created by a Kaggle user, was implemented. The warm-up scheduler uses the first 5 epochs of the model as a kind of 'warm up', increasing the training rate slowly to the desired value, after which it hands it off to the normal Cosine scheduler to continue the learning rate scheduling. This warm-up helps stabilize the training of the model, which in turn allows for overall better generalization within the model, leading to better performance, and allows for a high beginning learning rate value to be set for the learning rate scheduler, which gives way for faster convergence.

Lastly, to help further prevent overfitting, save computation time, and achieve faster convergence, early-stopping was implemented with the monitoring of the validation loss with a threshold value of 0.0001 and patience of 10.
...
The optimizer, on the other hand, was completely overhauled, with the Adam optimizer being replaced with Stochastic Gradient Descent (SGD). The reason for this change is that Adam operates better when the dataset quantity is lower, as it can converge quickly on smaller complex datasets and produce respectable results with less fine-tuning. Therefore, in the case of the progress report, this was ideal as it was only dealing with a fraction of a dataset; hence, an optimizer that is able to find the important generalized trends/patterns in a smaller amount of data was ideal.
...
\begin{center}
    \includegraphics[width=0.9\linewidth]{Images/RandomMatrix.png}
...
\begin{center}
    \includegraphics[width=0.7\linewidth]{Images/CNV_random.png}
    \captionsetup{hypcap=false}
...
\begin{center}
    \includegraphics[width=0.7\linewidth]{Images/DME_random.png}
    \captionsetup{hypcap=false}
...
\begin{center}
    \includegraphics[width=0.7\linewidth]{Images/DRUSEN_random.png}
    \captionsetup{hypcap=false}
...
\begin{center}
    \includegraphics[width=0.7\linewidth]{Images/NORMAL_random.png}
    \captionsetup{hypcap=false}
...
\begin{center}
    \includegraphics[width=0.7\linewidth]{Images/ProgressMatrix.png}
    \captionsetup{hypcap=false}
